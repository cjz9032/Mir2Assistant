...
namespace Mir2Assistant
{
    public partial class MainForm : Form
    {
        ...
        private async Task prepareBags(MirGameInstanceModel instanceValue, CancellationToken _cancellationToken)
        {
            ...
            await NpcFunction.sellDrugs(instanceValue, "疗伤药");
            await NpcFunction.SaveItem(instanceValue, "远程老板", 0, 0, instanceValue.Items.Concat(instanceValue.QuickItems).Where(o => !o.IsEmpty && o.IsGodly).ToArray());
                ...
                while (true)
                {
                    ...
                    try
                    {
                        ...
                        if (CharacterStatus.CurrentHP > 0)
                        {
                            ...
                            {
                                var sp = (instance.AccountInfo!.role == RoleType.mage) ? 105 : 100;
                            }
                            else
                            {
