...
public static class CharacterStatusFunction
{
    ...
    public static void GetInfo(MirGameInstanceModel gameInstance, bool force = false)
    {
        ...
        status.NameColor = memoryUtils.ReadToInt(memoryUtils.GetMemoryAddress(GameState.MirConfig["角色基址"], GameState.MirConfig["人物NAMEColor偏移"]));
        ...
        status.turn = memoryUtils.ReadToInt8(memoryUtils.GetMemoryAddress(GameState.MirConfig["角色基址"], GameState.MirConfig["人物TURN偏移"]));

        if (GameState.MirConfig["人物MinDef偏移"] != 0)
        {
            status.MinDef = memoryUtils.ReadToInt8(memoryUtils.GetMemoryAddress(GameState.MirConfig["角色基址"], GameState.MirConfig["人物MinDef偏移"]));
            status.MinMageDef = memoryUtils.ReadToInt8(memoryUtils.GetMemoryAddress(GameState.MirConfig["角色基址"], GameState.MirConfig["人物MinMageDef偏移"]));
            status.MaxDef = memoryUtils.ReadToInt8(memoryUtils.GetMemoryAddress(GameState.MirConfig["角色基址"], GameState.MirConfig["人物MaxDef偏移"]));
