...
namespace Mir2Assistant.TabForms.DailyTask
{
    ...
    public class DailyTaskService
    {
        ...
        string[] availableSkills = "裂神符,冰霜雪雨,灭天火,寒冰掌".Split(',');
        ...
        public void RunTask()
        {
            ...
            Task.Run(() =>
            {
                while (!Complete)
                {
                    ...
                    {
                        lockedMonster = gameInstance.Monsters.FirstOrDefault(o => o.Flag != 0 && TaskMonster.Keys.Contains(o.Name!));
                    }
                    else
                    {
                        lockedMonster = gameInstance.Monsters.FirstOrDefault(o => o.Addr == lockedMonster.Addr);
                    }
                    if (lockedMonster != null)
                    {
                        if (!stopRun)
